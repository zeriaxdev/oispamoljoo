...
function askForSnd() {
    let dialog = confirm("Haluutko musaa tähän?");
    if (dialog) {
        console.log(":DDD");
    } else {
        console.log("DDD:");
...
dogeSayings = [
    ...
    ":D",
    "ui juma!!",
    "ei viddu ku bärisee XD",
    "Keuli!",
    "Yhet vielä!",
    "ES pärisee Jonnet tärisee!!",
    "Löysin 5e s-marketin pihast!",
    "Fobbaa karkuun :D",
    "Vidun ykä!",
    "Ylilauta 5/5",
    "Huutista :--DD",
    ...
    "eiku kylille!!",
    "vidun assburger :DDD",
    "Sydänkohtaus tulille XD",
    "Kohta kofeiinimyrkytys :-D",
];
...
HTMLActuator.prototype.updateScore = function(score) {
    ...
    if (this.score > 1000) {
        this.katkoContainerColor.setAttribute(
            "style",
    ...
    if (difference > 0) {
        snd = new Audio(
            `${sndir}/open/open` + Math.floor(Math.random() * 3).toString() + ".ogg"
        );
        ...
        snd = new Audio(
            `${sndir}/none/none` + Math.floor(Math.random() * 3).toString() + ".ogg"
        );
    }
...
HTMLActuator.prototype.message = function(won) {
    ...
    var message = won ? "Maksimaalinen pärinä saavutettu!" : "D:";
...
HTMLActuator.prototype.goKatko = function() {
    snd = new Audio(`${sndir}/katkolle.ogg`);
    snd.play();
    ...
    var styleString = left + top + width;
    messageElement.setAttribute("style", styleString);
